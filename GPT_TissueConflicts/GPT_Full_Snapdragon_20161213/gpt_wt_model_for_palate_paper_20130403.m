function m = gpt_wt_model_for_palate_paper_20130403( m )
%m = gpt_wt_model_for_palate_paper_20130403( m )
%   Morphogen interaction function.
%   Written at 2015-08-17 12:36:51.
%   GFtbox revision 5276, 2015-08-12 17:10.
...
%%% USER CODE: INITIALISATION
    ...
    if m.globalDynamicProps.currentIter==0 % the first iteration
        ...
        m.userdata.ranges.modelnumber.range{2}='Delete cenorg and plt/lip growth'; % Reconfiguring model to introduce Kper growth at rim
        m.userdata.ranges.modelnumber.range{3}='Integrating wedge model into submodel 2'; % Reconfiguring model to introduce Kper growth at rim
        m.userdata.ranges.modelnumber.range{4}='Fig 9B Model 7M1'; % mutant of 'Fig 9B Model 7'
        m.userdata.ranges.modelnumber.range{5}='Fig 9B Model 7M2'; % mutant of 'Fig 9B Model 7'
        m.userdata.ranges.modelnumber.range{6}='Fig 9B Model 7M3'; % mutant of 'Fig 9B Model 7'
                
        %****************************** CHANGE THIS INDEX ******************************
        m.userdata.ranges.modelnumber.index=3; % selects one of these submodels, 
    ...
    switch (modelnumber) % setup the default morphogen activities (0 means zero activity, i.e. mutant)
        case {'Fig 9B Model 7', 'Delete cenorg and plt/lip growth', 'Integrating wedge model into submodel 2'} % we now add dich effect on distorg
...
%         end
        ...
        if abs(realtime - 240) <0.5*dt %At 240 hrs
            ...
            %expression domain of id_lipcliff established
            id_lipcliff_p=zeros(size(id_lipcliff_p));
            id_lipcliffnodes = m.nodes(:,3) > pd_offset-0.045 & m.nodes(:,3) < pd_offset-0.02;
            ...
            %expression domain of id_lipbend established
            id_lipbendnodes = m.nodes(:,3) > pd_offset -0.03 & m.nodes(:,3) < pd_offset-0.01;
            id_lipbend_p(id_lipbendnodes) = 1;
            m.morphogenclamp(id_lipbendnodes,id_lipbend_i) = 1;
        ...
        %expression domain of id_latquart established
            id_latquart_p=zeros(size(id_latquart_p));
            id_latquart_p = s_lat_p < 0.16 & s_med_p < 0.16;%-0.035 
            id_latquart_l = id_latquart_p * id_latquart_a;
           % m.morphogenclamp(id_latquart_i) = 1;
       ...
       if strcmpi(modelnumber,'Fig 9B Model 7') || strcmpi(modelnumber,'Fig 9B Model 7M2') || strcmpi(modelnumber,'Fig 9B Model 7M3') 
            % P EQUATIONS
...
%                 DKbper = inh (1, id_early_p .* id_div_l .* id_lpb_l.* inh (3, id_med_l));%bend out lips;   Eqn 48
             ...
             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        elseif strcmpi(modelnumber,'Delete cenorg and plt/lip growth') || strcmpi(modelnumber,'Fig 9B Model 7M2') || strcmpi(modelnumber,'Fig 9B Model 7M3') 
            % P EQUATIONS
            % START POLARISER section (it is slightly mixed with gene networks)
            m = leaf_mgen_conductivity( m, 's_distorg', 0.002); %was 0.1 distorg_diff);% diffusion constant
            m = leaf_mgen_absorption( m, 's_distorg', 0.05);  % it will not decay everywhere
...
%                 m.userdata.all_regions_ready=true;
            ...
            m.mgen_production(:,polariser_i) = 0.1 * (id_proxorg_l + 1 - P .* (id_distorg_l+0*id_late_p.*id_cenorg_l)); % Eqn 19 decay part of WAS S_DISTORG BY MISTAKE
            %m.mgen_production(:,polariser_i) = m.mgen_production(:,polariser_i) - 0.1 * P .* (id_distorg_l+3*id_late_p.*id_cenorg_l);  % Eqn 19 decay part of WAS S_DISTORG BY MISTAKE
...
%                 DKbper = inh (1, id_early_p .* id_div_l .* id_lpb_l.* inh (3, id_med_l));%bend out lips;   Eqn 48
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
             ...
             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        elseif strcmpi(modelnumber,'Integrating wedge model into submodel 2') || strcmpi(modelnumber,'Fig 9B Model 7M2') || strcmpi(modelnumber,'Fig 9B Model 7M3') 
            % P EQUATIONS
            % START POLARISER section (it is slightly mixed with gene networks)
            m = leaf_mgen_conductivity( m, 's_distorg', 0.002); %was 0.1 distorg_diff);% diffusion constant
            m = leaf_mgen_absorption( m, 's_distorg', 0.05);  % it will not decay everywhere
